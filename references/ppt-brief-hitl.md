...
内容表达方式、证据优先级、金字塔逻辑、页面标题质量和边界表述原则，统一遵循 `references/evidence-principle.md` 和 `references/evidence-examples.md`。
...
Summary Page 候选必须固定使用以下表达范式：

- `页面标题`：`<研究主体/公司>发布了<论文/技术/博客名称>`。标题只交代谁发布了什么，不在标题里塞收益、机制或评价，研究主体/公司要展现作者的权威性，优先用大公司，知名研究机构，如Google DeepMind，OpenAI，字节跳动等。
- `标题说明`：用 SMART 方式解释技术效果，必须包含可检视的对象、适用场景、量化指标、对照基线、时间窗口或实验/业务条件；优先写“对谁/什么对象，在什么条件下，相比什么，带来什么可衡量变化，边界是什么”。技术论文可写模型、数据集、硬件、配置和质量影响；业务/产品材料可写用户群、流程环节、转化/成本/时延/风险指标和统计口径。
- `分析总结`：固定只写两条，label 必须分别是 `机制` 和 `效果`。`机制` 解释技术如何达成效果；`效果` 解释可量化收益和成立口径。
...
- `正文内容`、`参考图片` 必须作为顶格字段出现，字段名前不能有空格、列表符号或缩进；扩写正文时，正文块的前四个字必须是 `正文内容`。
- Summary Page 密度最高，要同时压缩顶层结论、章节逻辑、关键数字/机制、决策含义、视觉线索和边界提醒。
- 内容页正文围绕该页 `分析总结` 展开。每个 `分析总结` label 必须在 `正文内容` 中再次出现，并写成独立 anchor 块：`【场景】`、`【机制】`、`【结果】`。anchor 后面再展开机制、事实、比较、例子、影响或边界；不要继续用 `- 场景：...` 作为正文一级结构。

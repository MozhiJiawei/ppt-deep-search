...
def validate(
    ...
    for page in pages:
        ...
        support_match = re.search(r"支撑信息[：:]?\s*\n(?P<body>.*?)(?:\n(?:边界提醒|信息密度说明)[：:]|\Z)", page.body, flags=re.S)
        support_items = []
        if support_match:
            support_items = re.findall(r"^\s*-\s*\S+", support_match.group("body"), flags=re.MULTILINE)
        if len(support_items) < 3:
            errors.append(f"{page.title} (line {page.start_line}) needs at least 3 supporting-info bullets for PPT material density")
    ...
    if not re.search(r"(顶层观点页|Top-level conclusion page|top-level conclusion page)", pyramid_outline, flags=re.I):
        errors.append("Pyramid Outline must include a standalone top-level conclusion page")
...
## Pyramid Outline
0. 顶层观点页：记忆层是 Agent 长程任务能力的基础设施。
   页面标题：长程任务需要可治理的记忆层
